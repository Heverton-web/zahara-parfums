...
export default function CardProduto({ produto, onWhatsAppClick }) {
  return (
    <div className="luxury-card rounded-xl sm:rounded-2xl overflow-hidden group">
      {/* Image container */}
        ...
        <div className="aspect-square bg-noir-900 relative overflow-hidden">
          {produto.imagem_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              <Gem className="text-gold/20" size={40} />
            </div>
          )}
          ...
          {produto.tags?.length > 0 && (
            <div className="absolute top-2 left-2 sm:top-4 sm:left-4 flex gap-1 sm:gap-2">
              {produto.tags.slice(0, 2).map((tag) => (
                <Badge key={tag} variant={tagColors[tag] || 'default'} className="text-[8px] sm:text-[10px] px-1.5 sm:px-2.5 py-0.5">
                  {tag}
                </Badge>
              ))}
            </div>
      ...
      <div className="p-3 sm:p-5">
        {/* Brand */}
        <p className="font-accent text-[8px] sm:text-[10px] uppercase tracking-wider text-gold/40 sm:text-gold/50 mb-1 sm:mb-2">
          {produto.marcas?.nome}
        </p>
        
        {/* Name */}
        <Link to={`/produto/${produto.id}`}>
          <h3 className="font-heading text-sm sm:text-base font-bold text-ivory mb-2 sm:mb-3 group-hover:text-gold transition-colors duration-300 line-clamp-2 leading-snug">
            {produto.nome}
          </h3>
        </Link>
        
        {/* Price */}
        <p className="price-tag text-base sm:text-xl mb-3 sm:mb-4">
        ...
        <button
          ...
          className="w-full btn-whatsapp text-xs sm:text-sm py-2.5 sm:py-3"
        >
